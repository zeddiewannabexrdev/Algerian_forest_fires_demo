...
def create_gauge_chart(score: float, level_info: dict, palette: dict) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={
            ...
            "font": {"size": 18, "color": text_color, "family": palette.get("font_title", "Montserrat, sans-serif")}
        },
        number={"suffix": "%", "font": {"size": 38, "color": text_color, "family": "Consolas, monospace"}},
        gauge={
            "axis": {"range": [0, 100], "tickwidth": 1, "tickcolor": sub_color, "tickfont": {"size": 11, "color": sub_color, "family": palette.get("font_body", "Lato, sans-serif")}},
    ...
    fig.update_layout(
        ...
        font={"family": palette.get("font_body", "Lato, sans-serif")}
    )
    return fig
...
def render_alert_view(df: pd.DataFrame, dt_model, rf_model, palette: dict):
    ...
    st.markdown(f"<h4 style='color:{text_pri}; margin:0; font-family:{font_title};'>GIÁM SÁT RỦI RO & CẢNH BÁO TỨC THỜI</h4>", unsafe_allow_html=True)
    ...
    with col_input1:
        st.markdown(f"<div style='font-family: {font_title}; font-size: 13px; font-weight: 700; color: {text_pri}; text-transform: uppercase; margin-bottom: 8px; border-bottom: 1px solid {border_col}; padding-bottom: 4px;'>Quan Trắc Khí Tượng Thực Địa</div>", unsafe_allow_html=True)
    ...
    with col_input2:
        st.markdown(f"<div style='font-family: {font_title}; font-size: 13px; font-weight: 700; color: {text_pri}; text-transform: uppercase; margin-bottom: 8px; border-bottom: 1px solid {border_col}; padding-bottom: 4px;'>Chỉ Số Vật Liệu Cháy (FWI System)</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='font-family: {font_title}; font-size: 13px; font-weight: 700; color: {text_pri}; text-transform: uppercase; margin-bottom: 8px; border-bottom: 1px solid {border_col}; padding-bottom: 4px;'>Chỉ Số Rủi Ro Tổng Hợp & Đánh Giá Độc Lập</div>", unsafe_allow_html=True)
    ...
    with col_models:
        ...
        <div style="background-color: {card_bg}; border: 1px solid {border_col}; border-left: 4px solid #475569; padding: 12px 14px; border-radius: 4px; margin-bottom: 10px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-family: {palette['font_title']}; font-size: 12px; font-weight: 700; color: {text_sec}; letter-spacing: 0.5px; text-transform: uppercase;">Decision Tree Engine</span>
                <span style="font-family: Consolas, monospace; font-size: 13px; font-weight: 700; color: {text_pri};">P = {dt_prob * 100:.1f}%</span>
            </div>
            <div style="font-family: {palette['font_title']}; font-size: 16px; font-weight: 700; color: {text_pri}; margin-top: 4px;">{dt_text}</div>
        </div>
        ...
        <div style="background-color: {card_bg}; border: 1px solid {border_col}; border-left: 4px solid {level_info['color']}; padding: 12px 14px; border-radius: 4px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-family: {palette['font_title']}; font-size: 12px; font-weight: 700; color: {text_sec}; letter-spacing: 0.5px; text-transform: uppercase;">Random Forest Engine</span>
                <span style="font-family: Consolas, monospace; font-size: 13px; font-weight: 700; color: {text_pri};">P = {rf_prob * 100:.1f}%</span>
            </div>
            <div style="font-family: {palette['font_title']}; font-size: 16px; font-weight: 700; color: {text_pri}; margin-top: 4px;">{rf_text}</div>
            <div style="font-family: {palette['font_body']}; font-size: 12px; color: {text_sec}; margin-top: 4px; font-weight: 600;">Độ đồng thuận: {tree_agree['fire_votes']}/{tree_agree['total_trees']} cây ({tree_agree['agreement_ratio']:.0f}%)</div>
        </div>
    ...
    st.markdown("<div style='height: 12px;'></div>", unsafe_allow_html=True)
    st.markdown(f"""
    <div style="border: 1px solid {border_col}; border-top: 4px solid {level_info['color']}; border-radius: 4px; padding: 14px 18px; background-color: {card_bg}; font-family: {palette['font_body']};">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
            <span style="font-family: {palette['font_title']}; font-weight: 700; color: {text_pri}; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px;">Chỉ Thị Tác Chiến: {level_info['name']}</span>

...
def render_explainability_view(df: pd.DataFrame, dt_model, rf_model, evaluator, palette: dict):
    ...
    st.markdown(f"<h4 style='color:{text_pri}; margin:0;'>GIẢI MÃ TRỌNG SỐ MÔI TRƯỜNG & ĐỐI CHIẾU THUẬT TOÁN</h4>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='font-size: 13px; font-weight: 700; color: {text_pri}; text-transform: uppercase; margin-bottom: 8px; border-bottom: 1px solid {border_col}; padding-bottom: 4px;'>Hiệu Năng Đo Lường (Benchmark Matrix)</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='font-size: 13px; font-weight: 700; color: {text_pri}; text-transform: uppercase; margin-bottom: 8px; border-bottom: 1px solid {border_col}; padding-bottom: 4px;'>Trọng Số Đóng Góp Của Các Đặc Trưng (Feature Importance)</div>", unsafe_allow_html=True)
    ...
    fig_imp.update_layout(
        barmode="group",
        xaxis=dict(title=dict(text="Thuộc tính", font=dict(color=text_pri)), gridcolor=grid_col, tickfont=dict(color=text_sec)),
        yaxis=dict(title=dict(text="Tỷ lệ trọng số (%)", font=dict(color=text_pri)), gridcolor=grid_col, tickfont=dict(color=text_sec)),
        height=360,
    ...
    st.markdown("<div style='height: 12px;'></div>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size: 13px; font-weight: 700; color: {text_pri}; text-transform: uppercase; margin-bottom: 8px; border-bottom: 1px solid {border_col}; padding-bottom: 4px;'>Cấu Trúc Cây Quyết Định & Hệ Thống Luật Suy Luận</div>", unsafe_allow_html=True)
    ...
    with tab_rules:
        ...
        for idx, row in df_display.iterrows():
            ...
            st.markdown(f"""
            <div style="background-color: {card_bg}; border: 1px solid {border_col}; border-left: 4px solid {tag_border}; border-radius: 4px; padding: 12px 16px; margin-bottom: 10px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-weight: 700; color: {text_pri}; font-size: 14px;">QUY TẮC {idx + 1}: KẾT LUẬN {row['prediction']}</span>
